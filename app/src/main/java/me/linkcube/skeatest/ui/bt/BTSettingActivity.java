	...
	private class ConnectDeviceAsyncTask extends
		...
		protected void onPostExecute(Boolean success) {
			...
			if (success) {
				...
				AlertUtils.showToast(BTSettingActivity.this, getResources().getString(R.string.toast_connect_toy_success), Toast.LENGTH_LONG);
				// 保存连接上的设备名和状态
				PreferenceUtils.setString(DEVICE_NAME, mDevice.getName());
				PreferenceUtils.setString(DEVICE_ADDRESS, mDevice.getAddress());
				deviceAdapter.notifyDataSetChanged();
			} else {
				AlertUtils.showToast(BTSettingActivity.this, getResources().getString(R.string.toast_connect_toy_failure), Toast.LENGTH_LONG);
			}
